...
export const WorthyClients = () => {
  ...
  return (
    <div className="relative -w-full overflow-x-hidden h-24 bg-[#F2F1F1]">
      <motion.div
        className="absolute inset-0 flex gap-4 py-4 bg-[#F2F1F1]"
        style={{ x: xTranslation }}
        ref={ref}
      >
        {[...logos, ...logos].map((logo, index) => (
          <motion.div key={index} className="relative h-full aspect-video">
            <Image
              fill
